...
void fixup_function_pointers( void )
{
	...
	alCaptureDestroy =
	    ( ALboolean( * )( void ) ) GP( "alCaptureDestroy_EXT" );
	...
	talBufferWriteData =
	    ( PFNALBUFFERWRITEDATAPROC ) GP( "alBufferWriteData_LOKI" );
	...
	talBufferAppendData =
	    ( ALuint( * )( ALuint, ALenum, ALvoid *, ALint, ALint ) )
	    GP( "alBufferAppendData_LOKI" );
	talBufferAppendWriteData =
	    ( ALuint( * )( ALuint, ALenum, ALvoid *, ALint, ALint, ALenum ) )
	...
	talutLoadRAW_ADPCMData = ( ALboolean( * )
				   ( ALuint bid, ALvoid * data, ALuint size,
	...
	talutLoadIMA_ADPCMData = ( ALboolean( * )
				   ( ALuint bid, ALvoid * data, ALuint size,
				     ALuint freq, ALenum format ) )
	...
	talutLoadMS_ADPCMData = ( ALboolean( * )
				  ( ALuint bid, ALvoid * data, ALuint size,
				    ALuint freq, ALenum format ) )
