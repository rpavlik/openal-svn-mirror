...
ALAPI ALvoid ALAPIENTRY alGetListener3f( ALenum pname, ALfloat* v1, ALfloat* v2, ALfloat* v3 )
{
	...
	{
		case AL_POSITION:
			*v1 = gListener.Position[0];
			*v2 = gListener.Position[1];
			*v3 = gListener.Position[2];
			break;
		case AL_VELOCITY:
			*v1 = gListener.Velocity[0];
			*v2 = gListener.Velocity[1];
			*v3 = gListener.Velocity[2];
			break;
		default:
			alSetError(AL_INVALID_ENUM);
			break;
...
ALAPI ALvoid ALAPIENTRY alGetListenerfv( ALenum pname, ALfloat* values )
{
	...
	{
		case AL_POSITION:
			values[0]=gListener.Position[0];
			values[1]=gListener.Position[1];
			values[2]=gListener.Position[2];
			break;
		case AL_VELOCITY:
			values[0]=gListener.Velocity[0];
			values[1]=gListener.Velocity[1];
			values[2]=gListener.Velocity[2];
			break;
		case AL_ORIENTATION:
			values[0]=gListener.Forward[0];
			values[1]=gListener.Forward[1];
			values[2]=gListener.Forward[2];
			values[3]=gListener.Up[0]; 
			values[4]=gListener.Up[1];
			values[5]=gListener.Up[2];
			break;
		default:
			alSetError(AL_INVALID_ENUM);
			break;
